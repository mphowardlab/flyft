...
void bindMesh(py::module_& m)
    ...
    py::class_<Mesh,std::shared_ptr<Mesh>,MeshTrampoline>(m, "Mesh")
        ...
        .def("lower", &Mesh::lower)
        .def("upper", &Mesh::upper)
        .def("lower_boundary_condition", &Mesh::lower_boundary_condition)
        .def("upper_boundary_condition", &Mesh::upper_boundary_condition)
