...
Vext = flyft.external.CompositeExternalPotential((flyft.external.HardWall(padding,1.0),
...
for t in state.fields:
    ...
    inside = np.logical_and(x >= Vext[0].origin + 0.5*d,
                            x <= Vext[1].origin - 0.5*d)
    Linside = (Vext[1].origin-Vext[0].origin-d)
    state.fields[t][inside] = ((rho-0.5*drho)+(drho/Linside)*(x[inside]-Vext[0].origin+0.5*d))
...
euler = flyft.dynamics.ExplicitEulerIntegrator(timestep=1.e-3)
