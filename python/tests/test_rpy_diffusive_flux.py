...
def spherical_mesh_grand():
    return flyft.state.SphericalMesh(10.0, 1000, "repeat")
...
def test_ideal(grand, ig, rpy, state_grand):
    ...
    assert np.allclose(rpy.fluxes["A"], 0.0)
    ...
    r"""The chemical potential of the ideal gas equation of state given by:
    ...
    mobility was done in Mathematica and the equations for the two different
    ...
    for i in range(len(x)):
        ...
        if left_edge == 0:
            ...
            bd = 1 / (3 * np.pi * 1.0 * 1.0) * 3.0 / R
            M = left_edge * (-5 + left_edge**2) / (18) * 3.0 / R
        else:
            # Limits of integration x-(ai+ak) to x+(ai+ak), where ai = ak = 0.5
            bd = 1 / (3 * np.pi * 1.0 * 1.0) * 3.0 / state.mesh.full.L
            r"""Integrated form of the mobility is equal to \int{M}\d y for
...
def test_excess(grand, ig, rpy, state_grand):
    ...
    for i in range(len(x)):
        ...
        if left_edge == 0:
            ...
            r"""The chemical potential of the virial equation of state given by:
            ...
            Using this the above mentioned procedure, calculation can be done
            analytically, considering the different limits of the mobility
            tensor as seen in the ideal gas calculation. The calculation of the
            flux was done in Mathematica and the equations for the two different
            limits of integration as mentioned in the ideal gas calculation are
            implemented below.
            """
            ...
            bd = -0.031831 * (1 + 3 * left_edge)
            M = (1 / left_edge) * (
                -0.005 + left_edge * (left_edge * (0.025 + left_edge * (0.075)))
            )
        flux[i] = M + bd
        lower_bounds[i] = left_edge
...
def test_binary_ideal(grand, ig, rpy, binary_state_grand):
    ...
    rpy.compute(grand, state)
    assert np.allclose(rpy.fluxes["A"], 0.0)
    assert np.allclose(rpy.fluxes["B"], 0.0)
