...
def test_ideal(grand,ig,rpy,state_grand):
    ...
    for i in range(len(x)):
        ...
        if i==0:
            ...
            Using this the above mentioned procedure, calculation can be done analytically for a 
            given density field. The calculation of the integration of the mobility was done in Mathematica and the 
            equations for the two different limits of integration calculation are implemented below.
            """
...
def test_binary_ideal_one(grand,ig,rpy,binary_state_grand):
    """In this test we are checking if a "two component" system of same size would 
    generate the same result as the one-compenent system. Same procedure is followed 
    as shown in the ideal gas equation of state calculation however in this case 
    two different types are considered with same particle radius ,i.e. ai = ak = 0.5. 
    The resulting fluxes calculated should match the one-component ideal gas fluxes.
    The flux equations are computed in Mathematica and implemented below"""
...
def test_binary_ideal(grand,ig,rpy,binary_state_grand):
    ...
    considered ,i.e. ai = 0.5 and ak = 1.5. The fluxes are computed in Mathematica 
