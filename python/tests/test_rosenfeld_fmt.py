...
def muex_py(eta):
    """Percus-Yevick excess chemical potential of hard spheres \
    compressibility route)"""
    return 0.5 * (14 * eta - 13 * eta**2 + 5 * eta**3) / (1 - eta) ** 3 - np.log(
        1 - eta
    )
...
def test_compute(fmt, binary_state):
    ...
    fmt.compute(state)
    assert fmt.value == pytest.approx(volume * fex_py(eta, v), abs=1e-3)
    assert np.allclose(fmt.derivatives["A"].data, muex_py(eta), atol=1e-3)
    ...
    assert fmt.value == pytest.approx(volume * fex_py(eta, v), abs=1e-3)
    assert np.allclose(fmt.derivatives["A"].data, muex_py(eta), atol=1e-3)
    assert np.allclose(fmt.derivatives["B"].data, muex_py(eta), atol=1e-3)
