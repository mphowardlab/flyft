...
def test_compute(grand, state):
    ...
    grand.compute(state)
    assert grand.value == pytest.approx(
        volume * (f_ig(rho, 1.0) + fex_py(eta, v)), abs=1e-2
    )
    assert np.allclose(
        grand.derivatives["A"].data, mu_ig(rho, 1.0) + muex_py(eta), atol=1e-2
    )
    ...
    grand.compute(state)
    assert grand.value == pytest.approx(
        volume * (f_ig(rho, 1.0) + fex_py(eta, v) - mu_bulk * rho), abs=1e-2
    )
    assert np.allclose(
        grand.derivatives["A"].data, mu_ig(rho, 1.0) + muex_py(eta) - mu_bulk, atol=1e-2
    )
