...
def test_ideal(grand,ig,bd):
    ...
    state.fields['A'][:] = 1.
    grand.constrain('A', mesh.L*1.0, grand.Constraint.N)
    bd.compute(grand,state)
    assert np.allclose(bd.fluxes['A'], 0.)
    ...
    bd.compute(grand,state)
    assert np.allclose(bd.fluxes['A'][1:], -2.0*3.0/mesh.L)

    # make sinusoidal profile and test with looser tolerance due to finite diff
    state.fields['A'][:] = (np.sin(2*np.pi*x/mesh.L)+1)
    grand.constrain('A', mesh.L*1., grand.Constraint.N)
    bd.compute(grand,state)
    # flux is computed at the left edge
    j = -2.0*(2.*np.pi/mesh.L)*np.cos(2*np.pi*(x-0.5*mesh.step)/mesh.L)
...
def test_excess(grand,ig,fmt,bd):
    ...
    grand.constrain('A', mesh.L*1.e-2, grand.Constraint.N)
    bd.compute(grand,state)
    assert np.allclose(bd.fluxes['A'], 0.)

    # TODO: replace this with a local hs functional instead of fmt so the test is exact
    x = mesh.coordinates
    state.fields['A'][:] = 1.e-1*(np.sin(2*np.pi*x/mesh.L)+1)
    grand.constrain('A', mesh.L*1.e-1, grand.Constraint.N)
    bd.compute(grand,state)
    # flux is computed at the left edge
    d = fmt.diameters['A']**3
    v = np.pi*d**3/6.
    rho = 1.e-1*(np.sin(2*np.pi*(x-0.5*mesh.step)/mesh.L)+1)
    drho_dx = 1.e-1*(2.*np.pi/mesh.L)*np.cos(2*np.pi*(x-0.5*mesh.step)/mesh.L)
    eta = rho*v
