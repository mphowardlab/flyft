...
@pytest.fixture
def cn():
    return flyft.dynamics.CrankNicolsonIntegrator(1.0e-3, 1.0, 2, 1.0e-6)


def test_init(cn):
    assert cn.timestep == pytest.approx(1.0e-3)
    assert cn.mix_parameter == pytest.approx(1.0)
    assert cn.max_iterations == 2
    assert cn.tolerance == pytest.approx(1.0e-6)

    # change time step
    cn.timestep = 1.0e-2
    assert cn.timestep == pytest.approx(1.0e-2)
    ...
    # change tolerance
    cn.tolerance = 1.0e-7
    assert cn.tolerance == pytest.approx(1.0e-7)
    assert cn._self.tolerance == pytest.approx(1.0e-7)
